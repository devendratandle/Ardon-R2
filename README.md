...
| **[`benchmarks/llm/REPORT.md`](benchmarks/llm/REPORT.md)** | **R2 vs PyTorch and JAX on LLM training** — the single source for those numbers. A 7.24M-parameter model trained on TinyStories trains **1.3× faster than PyTorch 2.13+MKL** and **learns identically** (same loss to four decimals at every checkpoint, from shared initial weights; forward 1.48× and backward 1.41× faster phase by phase). BPE tokenization is **~11× faster than HuggingFace `tokenizers`**. Includes what was tried and measured *worse*, so it isn't retried. |
...
- **LLM training that keeps up with PyTorch** — BPE tokenizer, autograd tape, fused attention, and an Adam trainer, all in Rust with no C. Trains **1.3× faster than PyTorch+MKL** and learns identically; see [`benchmarks/llm/REPORT.md`](benchmarks/llm/REPORT.md).
