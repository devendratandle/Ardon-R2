...
| **J.4** | Compose source libraries to native: user-helper inlining, multi-reduction & vector-returning kernels for the statistics primitive set, with common-subexpression elimination, single-pass wave fusion, and F64X2 SIMD. A formula written once reaches/beats the native builtin. **Iterative kernels (whole-function slice 1):** a whole function with a counted loop carrying scalar state across per-iteration vector reductions — the gradient-descent / Newton / fixed-point / EM shape — compiles as one native unit (loop-carried scalars as SSA phis, fused waves per iteration, R-faithful `1:K` in both directions, read-before-define declines to the interpreter). A 200-iteration training loop over n=1e5 runs 8.4× faster than interpreted even in debug. *Remaining:* matrix-valued state (`%*%`/`cor(matrix)`/`scale` on a scratch arena — the r2sem `fit_once` bottleneck), scalar `if/else` between statements. | ◐ scalar/vector class + iterative kernels shipped; matrix state queued |
