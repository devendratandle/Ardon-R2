...
| Microkernel ② | BLAS/LAPACK | ◐ partial | LU, Cholesky, QR, SVD, symmetric eigen (`dsyev`) + **non-symmetric eigen (`dgeev`: Hessenberg + Francis double-shift QR)** — done. Pivoting QR, Lanczos, `backsolve`/`rcond`, complex type — todo |
