...
| **J.3** | matrices / list access unboxed inside compiled code; guards + deopt for speculated types. **First brick shipped:** matrix arguments are unboxed at the JIT dispatch by treating their column-major buffer as the dense vector the existing kernels already consume — `sum(m*m)` / `sqrt(m)` / `A+B` / `sum(A*B)` all JIT with **zero new codegen**, dim + dimnames preserved on maps, results bit-exact vs the interpreter (verified on 100×100 random). **Remaining:** `m[i,j]` scalar loads inside a loop, and list `x[[i]]` unboxing, need real indexed-load codegen + type guards/deopt. | data plumbing stops boxing | ◐ **in progress** |
