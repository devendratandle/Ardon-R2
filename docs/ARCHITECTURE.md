...
| **J.4** | inline hot user + builtin calls into the loop | kills per-call dispatch → **iterative source libraries (r2sem) go near-native** | ◐ **brick 1 shipped:** pure user-helper *inlining* — a numeric function composed of JIT-lowerable helpers (`function(a,b) sq(a)+sq(b)`, `sum(cube(v))`) is inlined by AST substitution (depth-bounded → recursion falls back) so it compiles as one unit. **Remaining (the r2sem-relevant part):** lower *builtin* matrix/reduction calls (`%*%`→dgemm, `cor`, `sum`) inside a JIT'd region onto unboxed buffers + a scratch arena, so whole functions like `fit_once` compile without per-op RVal allocation. |
