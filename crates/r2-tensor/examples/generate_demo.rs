...
fn main() {
    ...
    for (name, c) in [
        ("demo",  Config{dim:16,n_heads:4,n_layers:3,vocab:11,ffn_hidden:24,max_seq:32,rope_base:10000.0,eps:1e-5}),
        ("~7B",   Config{dim:4096,n_heads:32,n_layers:32,vocab:32000,ffn_hidden:11008,max_seq:4096,rope_base:10000.0,eps:1e-5}),
        ("~32B",  Config{dim:6656,n_heads:52,n_layers:60,vocab:32000,ffn_hidden:17920,max_seq:4096,rope_base:10000.0,eps:1e-5}),
    ] {
        let p = c.n_params();
    ...
    // Actually generate.
    let c = Config{dim:32,n_heads:4,n_layers:4,vocab:50,ffn_hidden:64,max_seq:64,rope_base:10000.0,eps:1e-5};
    ...
    for (i,l) in m.layers.iter_mut().enumerate(){ let s=i as f32*3.1;
        ...
        l.wq=f(c.dim*c.dim,s+0.2); l.wk=f(c.dim*c.dim,s+0.5); l.wv=f(c.dim*c.dim,s+0.9); l.wo=f(c.dim*c.dim,s+1.3);
