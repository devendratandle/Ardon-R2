...
| **Intel per-core `sgemm` probes** (i5-12500, 1 thread, 2026-09-25) — where does the single-thread gap to MKL (R2 0.82-0.93x) go? | The micro-kernel ALONE, L1-hot, runs **140 GFLOP/s** (~95% of the 4.6 GHz AVX2 peak) at every kc; inside a GEMM it reaches ~113 (960-2048 rows) and 99 on a single 96-row block (B packing amortised over too few rows). Three fixes measured nothing, each A/B interleaved: **KC 256/384/512** (mean ratio to MKL 0.88/0.90/0.91, noise); **software prefetch of the next B strip** (one line per k step, `_MM_HINT_T0`) — within ±2%, the hardware prefetcher already streams the contiguous strips; **B-strip-outer / A-panel-inner loop order** (BLIS) — same or 2-4% worse, as on Zen 2. The residual ~20% is spread over packing, the tile write-back and L2 traffic, not in one place. Not retried |
